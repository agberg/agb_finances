...
raw_data <- readr::read_csv("ynab-2023-07-13.csv") %>%
  janitor::clean_names() %>%
...
core_expenses <- short_term$output %>% dplyr::filter(category_group %in% c("Health (10%)", "Travel (4%)", "Fun Stuff (2%)", "Relationship (3%)", "Work (1.5%)"))

summary_of_core_expenses <- core_expenses %>% dplyr::ungroup() %>% dplyr::summarise(dplyr::across(.cols = -1 , .fns = ~sum(.x)))
